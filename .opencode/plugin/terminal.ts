...
const runInAppleTerminal = (worktreePath: string, sessionId: string) => {
	const script = `
on run argv
...
const runInITerm = (worktreePath: string, sessionId: string) => {
	const script = `
on run argv
  set worktreePath to item 1 of argv
  set sessionId to item 2 of argv
...
const runInAlacritty = (worktreePath: string, sessionId: string) => {
	const command = 'cd "$WORKTREE" && opencode --session "$SESSION"';

	spawnDetached(
		"open",
		["-a", "Alacritty", "--args", "-e", "zsh", "-lc", command],
		{
...
const openOnMacOS = (worktreePath: string, sessionId: string) => {
	...
	for (const c of candidates) {
		if (isMacAppInstalled(c.name)) {
			c.run(worktreePath, sessionId);
			return;
		}
	}
